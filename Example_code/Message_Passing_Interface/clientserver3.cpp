...
int main(int argc, char* argv[]) 
{
	...
	if (myRank != 0) {	// execute client clode
		...
		while(true) {
			...
			MPI_Waitsome(nProc-1, req, &numMsg, reqIndex, status);

			cout << "Handling " << numMsg << " received messaged" << endl;
			for (size_t i = 0; i < numMsg; i++) {
				int recvSize;
				MPI_Get_count(&status[i], MPI_CHAR, &recvSize);
				int srcRank = status[i].MPI_SOURCE;

				cout << "Handler request from process " << srcRank << " of size: " << recvSize << endl;
				handleRequest(buffer[reqIndex[i]], recvSize);

				MPI_Irecv(buffer[reqIndex[i]], MAX_SIZE, MPI_CHAR, srcRank, 0, MPI_COMM_WORLD, &req[reqIndex[i]]);
			}
		}
